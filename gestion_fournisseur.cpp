    ...
    ui(new Ui::gestion_fournisseur)
{
    ui->setupUi(this);
        ui->tableView->setModel((f.afficher()));
            gestionTeacherDialog = new gestion_teacher(this);
...
void gestion_fournisseur::on_Ajouter_clicked()
{
   ...
   QString CLASSE=ui->lineEdit_ADRESSE->text();
   QString ID=ui->lineEdit_MARCHANDISE->text();
   fournisseur f( CINF,  NOMF, PRENOMF, CLASSE, ID );
   bool test=f.ajouter();
   if(test)
       {
           ui->tableView->setModel((f.afficher()));
...
void gestion_fournisseur::on_Modifier_clicked()
{
   ...
   fournisseur f( CINF,  NOMF, PRENOMF, CLASSE, ID );
   bool test=f.modifier(CINF,  NOMF, PRENOMF, CLASSE, ID );
   if(test)
                   {
                       ui->tableView->setModel(f.afficher());
